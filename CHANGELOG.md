...
- **Web Widget: real email-code verification for "I am already a user," and a "you have a new reply" email for a visitor who has left.** Two pieces toward WhatsApp-level parity for the widget ahead of an Ionic app rollout (logged-in users there already work today via the existing signed in-app identity token — no new code needed for that half).
  - **Email-code verification** turns on the previously-stubbed `email_code` mode of Settings → Channels → Web Widget → Web verification (needs `RESEND_API_KEY` configured — the save is refused otherwise). A typed phone/email claim no longer attaches to (or creates) a contact directly: it's looked up against real contacts with **no side effect at all** unless it matches one that has an email on file, in which case a 6-digit code is emailed to that contact's own email (never the one typed) and the widget shows a code-entry screen. Confirming the code (`POST /api/widget/verify-code`) makes the browser `verified` — the same trust tier a signed in-app token gets, including the guest-fold-in and the two-real-contacts auto-merge rule. An unmatched claim, or a match with no email on file, gets no code and leaves no trace — deliberately, since the point of this mode is "must be a real, existing customer," not "attach to whatever was typed."
  - **"You have a new reply" email**, sent best-effort when an agent (or automation) replies on a web-widget conversation whose visitor is `verified` with an email and appears to have left (no `widget_visitors` activity in the last 2 minutes) — at most one per "away period," never for an unverified `claimed` identity (its email could be a stranger's). This is the cheap, always-available half of "how close to WhatsApp's own notify-on-reply can the widget get" — it needs no vendor beyond Resend and works even inside a mobile WebView, which cannot use the Web Push API at all; real push (Web Push for a browser tab, native push for a mobile app) is bigger, separately-scoped follow-on work, not built here.
  - Migration 110 adds two service-role-only tables: `widget_verification_codes` (one pending code per browser) and `widget_reply_notifications` (last-notified bookkeeping per conversation) — no changes to any existing table.

## [0.65.0] — 2026-09-26 — **migration required (109)**

- **Invite-by-email invites can now actually be emailed.** 0.64.1 shipped invite-by-email as auto-join-only ("no email is sent") because there was no outbound mail sender anywhere in the app. This adds one: set `RESEND_API_KEY` (and, for real use, a verified-domain `RESEND_FROM_EMAIL` — see `.env.local.example`) and `POST /api/account/invitations` sends the invite link via [Resend](https://resend.com) the moment an email-targeted invite is created. Best-effort and purely additive — an unconfigured or failing send never blocks invite creation, and the create-invite dialog always still shows the link to share manually; it now also reports whether the email actually went out (sent / not configured / send failed), and the Members list shows an "Email sent" badge on invitations it succeeded for. Migration 109 adds the one nullable `account_invitations.email_sent_at` column this is tracked on. New `src/lib/email/resend.ts` (a plain-`fetch` Resend client, no SDK — same convention as every other external API integration in this app) and `src/lib/email/invitation-email.ts` (the email content). See `docs/sso-login-setup.md`'s new "Email delivery (optional — Resend)" section.

## [0.64.1] — 2026-09-26 — **migration required (108)**

- **Invite by email.** Settings → Team → "Invite a teammate" now has an optional email address field. Set it and that exact, *verified* email auto-joins the invited account/role the moment it signs in — including immediately via Google/Microsoft SSO (already-verified by the provider), or once a password signup confirms its email — no link click or "Accept" step needed. The plain shareable-link invite is unchanged when the field is left blank. Migration 108: `account_invitations.email` (nullable, one pending invite per email per account), a new `attach_user_to_invited_account()` shared by the manual redeem RPC and two new triggers (`handle_new_user`'s existing bootstrap gets one additive branch; a brand-new `on_auth_user_email_confirmed` trigger covers the password-signup path — no trigger on `auth.users` UPDATE existed before this). If someone's own account already has real data under that email, auto-join is skipped rather than silently discarding it — the invite stays pending for manual resolution, same guard the link-accept flow has always had. No email is actually sent — there's no outbound transactional-email sender anywhere in this app, and standing one up is a separate infra decision; see `docs/sso-login-setup.md`'s new "Invite by email" section.

## [0.64.0] — 2026-09-26

- **Sign in with Google / Microsoft.** "Continue with Google"/"Continue with Microsoft" now appear on `/login` and `/signup`, alongside the existing password form (unchanged). No new database migration — this is pure Supabase Auth (GoTrue) OAuth, entirely separate from the Gmail/Microsoft 365 *channels* (those connect one shared mailbox for customer email; this is individual login, needs its own separate Google Cloud OAuth client and Azure AD app registration, and asks for nothing beyond `openid email profile`). New `/auth/callback` route exchanges the returned code for a session and honors an in-flight invite the same way the password flow does — clicking through from `/join/<token>` lands back on the invite-accept screen, not `/dashboard`. Requires turning the Google/Azure providers on in Supabase Auth before the buttons do anything — see `docs/sso-login-setup.md`. Invites are still link-based, not email-targeted, so this doesn't (yet) deliver "add someone's email, they land in the account automatically" — that's separate, larger scope, flagged explicitly in the setup doc rather than silently assumed.

## [0.63.4] — 2026-09-26 — **migration required (107)**

- **Sembang: link unfurling — preview cards for pasted URLs.** The first `http(s)` URL in a sent message gets a title/description/image card, fetched server-side after the message is already sent (never delays or blocks a send) via the same SSRF-guarded fetcher the Knowledge Base's "import a web page" feature already uses (`isDeliverableUrl()` — rejects private/internal/cloud-metadata targets, DNS-resolved, not just string-matched). Migration 107 adds `sembang_link_previews` (one row per message, written only by the server — no client write policy at all), joined into message hydration the same way attachments are, and pushed live via a new realtime subscription. A page with neither an OpenGraph title nor a `<title>` tag — or one that's unreachable, private, non-HTML, or too large — silently gets no card rather than a broken one.
- **Sembang: "Schedule a meeting" — real Google/Microsoft calendar invites.** The meeting button is now a menu: "Start an instant meeting" (unchanged) plus "Schedule a meeting…", which creates a real calendar event (Google Meet or Microsoft Teams link auto-attached) with every other channel member invited. Reuses the account's existing connected Gmail/Microsoft 365 channel end-to-end — same encrypted token storage, same refresh logic, no new OAuth flow and no new table. The event's organizer is that one shared connected mailbox, not the individual member scheduling it (a deliberate scope decision — an already-connected account needs one reconnect to grant the added `calendar.events`/`Calendars.ReadWrite` scope, since Google/Microsoft won't add a scope to an existing consent without going through it again).

## [0.63.3] — 2026-09-25 — **migration required (106)**

- **Sembang: an "X" to remove a DM from the sidebar.** Hover a DM row and click the X (same spot Slack puts it) to drop it out of your own sidebar list. This never deletes the conversation, its messages, or the other person's copy of it — only your own `sembang_channel_members` row is marked hidden (migration 106's `set_sembang_channel_hidden` RPC, modeled on the existing mute RPC's same "no client UPDATE policy, go through a self-scoped SECURITY DEFINER function" shape). It comes back automatically: a new message landing in it un-hides it again, and so does re-opening it (marking it read now also clears the hidden flag), so it can't be accidentally lost for good. Group channels aren't affected — they're left/archived, not hidden.

## [0.63.2] — 2026-09-25 — **migration required (105)**

- **Sembang P4: voice messages.** The last of the four P4 items scoped out of 0.63.0 ("Drafts + Files/Links/Bookmarks"). Reuses the Inbox composer's exact recording pipeline rather than building a second one: the mic-capture/Ogg-Opus-encode state machine (`opus-recorder`, unchanged encoder settings) was extracted out of `src/components/inbox/message-composer.tsx` into a shared `useVoiceRecorder` hook, and the Inbox composer itself was refactored to call it — same behavior, same strings, now one implementation instead of a soon-to-be-duplicated one. Sembang's composer uses the same hook plus the same `RecordingBar` UI (now parametrized by a `t` prop instead of a hardcoded i18n namespace) and its own existing `uploadSembangFile()` upload path — no new upload code needed. A voice note has no caption, same as WhatsApp's, so an attachment-only send needed two small server-side changes: migration 105 relaxes `sembang_messages.body`'s CHECK from requiring 1+ chars to allowing 0, and allow-lists `audio/ogg` on the private `sembang-files` bucket (previously picked-file MIME types only). The API route's own validation now requires body text OR at least one attachment, never neither. Playback is a plain `<audio controls>` (no waveform, matching the Inbox thread's own `MediaAudioBubble`) — attachments are branched by MIME type in `message-row.tsx`. Added `MessagePreview` (in `message-body.tsx`) so the four places that preview a message's text (Starred, Pins, Threads, Mentions) show a "Voice message" / filename fallback instead of a blank line when a message has no body, rather than duplicating that fallback four times.
...
- **Sembang: internal team chat (P0).** A new, separate "Sembang" menu item — Slack-style channels for internal engineering/team coordination, deliberately walled off from the customer-facing Inbox (its own tables, no foreign keys into `conversations`/`messages`/`tickets`). Public or private channels, @mentions with notifications, file attachments, and per-channel unread counts, on the same mobile list/thread collapse pattern as the Inbox. Any member with Sembang access can create a channel; the creator automatically becomes its **moderator** (can remove messages, and — for private channels — add members); account admins can see and moderate every channel regardless of membership.
  - Access is capability-gated (`menu.sembang`), default **Owner + Admin only** — an admin can grant it down to Agent from the existing Permissions screen, which is the "given access" mechanism. Unlike every other menu capability in this app, `menu.sembang` is enforced at the database level too (every `sembang_*` table's RLS calls `has_capability()` directly), because plain account membership is deliberately not enough to read Sembang.
  - New private Supabase Storage bucket `sembang-files` (16MB cap, same mime allowlist as `chat-media` plus zip/json/csv/markdown) — unlike `chat-media`, this one is **not** public; reads go through short-lived signed URLs.
  - Explicitly **not** in this pass (tracked for P1): threaded replies, reactions, pins, code blocks with syntax highlighting, the per-channel task list, editing/deleting your own message, typing indicators, and a "browse public channels" discovery screen (channels currently only appear once you're invited, either at creation or later by a moderator).
  - Migration 098 adds `sembang_channels`, `sembang_channel_members`, `sembang_messages`, `sembang_attachments`, the `menu.sembang` capability, two RPCs (`mark_sembang_channel_read`, `list_sembang_channels_for_current_user` — the latter does the unread-count aggregation so nothing needs a maintained counter column), a `sembang_mention` notification type, and the new storage bucket + policies.
