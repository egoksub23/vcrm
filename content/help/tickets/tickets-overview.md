...
description: See all tickets on a board or a list, and find the one you need.
order: 1
updated: 2026-09-21
---

A ticket is a job to follow up. Use one when a problem needs more than a chat reply, for example a refund check or a bug report. Each ticket has a key such as **VIR-12**, so you and your team can talk about the same one.

## Open the Tickets page

Click **Tickets** in the sidebar. At the top right, switch between **Board** and **List**. The app remembers your choice on this device.

The key starts with a short prefix. The default is VIR, and your admin can change it. Old numbers and links keep working after a change.
...
Each column is a status. The **Closed** column is folded away to save space. Click **Show Closed tickets** at the right edge to open it, and click **Hide column** to fold it again.

A card shows the summary, the customer, up to three labels, the ticket type, the key, the priority, the due date, an SLA badge, the number of comments and who it is assigned to. Click a card to open the ticket.
...
The filters are part of the page address, so you can send a link with them. Search and filters work on the tickets that are loaded. If a ticket is missing, use **Show more** or **Load more**.

A due date is amber on the day it is due and red once it is overdue.
...
- Start your day with **My tickets** and **Overdue**.
- Save the filters you use often.
- Copy the ticket link and paste it in a comment when you ask a teammate for help.
